...
//       onUpdate({ ...staff, status: previousStatus })
//     }
//   }
...
//     <div className="relative">
//       <motion.div
//         className="grid grid-cols-[2rem_1.5fr_2fr_1.5fr_1fr_auto] items-center gap-4 px-4 py-2 cursor-pointer hover:bg-muted/50"
//         onClick={onToggle}
//         initial={false}
//         animate={{ backgroundColor: isExpanded ? "hsl(var(--muted))" : "transparent" }}
//         transition={{ duration: 0.2 }}
...
//           <div className="truncate">{staff.phone}</div>
//         </div>
//         <div className="truncate">{staff.position}</div>
//         <div className="truncate">{staff.service_period}</div>
//         <div onClick={(e) => e.stopPropagation()} className="flex justify-center min-w-[120px]">
//           <Select defaultValue={staff.status} onValueChange={handleStatusChange}>
//             <SelectTrigger
//               className={`h-7 px-3 text-xs font-medium rounded-full ${getStatusStyle(staff.status)}`}
//               style={{ minWidth: "100px", maxWidth: "120px" }}
...
export function StaffCard({ staff, isExpanded, onToggle, onUpdate }: StaffCardProps) {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Normalize the position for "Waiter" or "Waitress"
      if (editedStaff.position === "Waiter" || editedStaff.position === "Waitress") {
        editedStaff.position = "Waiter/Waitress"
      }
